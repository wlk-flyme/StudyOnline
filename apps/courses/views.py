...
from operation.models import CourseFavorite, CourseComments, UserCourse
...
class CourseListView(View):
    def get(self, request):
        ...
        p = Paginator(all_courses, 6, request=request)  # 每页3个
...
class CourseDetailView(View):
    ...
    def get(self, request, course_id):
        ...
        if request.user.is_authenticated():
            if CourseFavorite.objects.filter(user=request.user, fav_id=course.id, fav_type=1):
                has_fav_course = True

            if CourseFavorite.objects.filter(user=request.user, fav_id=course.course_org.id, fav_type=2):
                has_fav_org = True
...
class VideoPlayView(LoginRequiredMixin, View):
    ...
    def get(self, request, video_id):
        # 此处的id为表默认为我们添加的值。
        video = Viedo.objects.get(id=int(video_id))
        # 找到对应的course
